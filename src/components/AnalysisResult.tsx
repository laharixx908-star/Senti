...
// Each emoji is a custom neon SVG — same reference style as the screenshots
function NeonEmoji({ emotion, color }: { emotion: string; color: string }) {
  const s = color; // stroke / fill color
  const glow = `drop-shadow(0 0 6px ${color}) drop-shadow(0 0 12px ${color})`; 
  ...
  switch (emotion) {
    ...
    case "Confident":
      return face(
        // slight smirk
        <path d="M19 29 Q26 34 33 29" stroke={s} strokeWidth="2.5" strokeLinecap="round" />
      );
    ...
    case "Heartbroken":
      return face(
        <path d="M16 32 Q24 26 32 32" stroke={s} strokeWidth="2.5" strokeLinecap="round" />,
        // broken heart
        <path d="M21 10 C21 8 19 6 17 8 C15 10 17 13 21 15 L24 13 L21 15 C25 13 27 10 25 8 C23 6 21 8 21 10Z M21 15 L24 13" stroke={s} strokeWidth="1.5" fill="none" />
      );
